...
export function TestBoardDisplay({ 
  ...
  const canvasToWorldPosition = (canvasX: number, canvasY: number): BoardPosition => {
    ...
    const gridX = Math.round((worldX - (CARD_WIDTH * TILE_WIDTH) / 2) / TILE_WIDTH);
    const gridY = Math.round((worldY - (CARD_HEIGHT * TILE_HEIGHT) / 2) / TILE_HEIGHT);
  ...
  const drawHighlights = (ctx: CanvasRenderingContext2D) => {
    if (!scoringResults?.highlightedTiles || scoringResults.highlightedTiles.length === 0) {
      return;
    }
  ...
  const [mouseGridPos, setMouseGridPos] = React.useState<{ x: number; y: number } | null>(null);
    ...
    if (mouseGridPos && selectedCardDef && canvasRef.current) {
      ...
      if (ctx) {
        ...
        const previewCard: Card = {
          ...
          rotation: 0,
          customMetadata: selectedCardDef.customMetadata,
        };
